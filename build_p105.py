#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""WeatherTool 105% 界面大小分支构建脚本（v9.90P）：
...
KS_PASS = "***REMOVED***"
BASE_APK = os.path.join(BUILD, "base_p105.apk")
OUT_APK = os.path.join(BUILD, "WeatherTool_P105.apk")
VER_CODE = "91"
VER_NAME = "9.90P"
OUT_VER_APK = os.path.join(BUILD, "WeatherTool_v" + VER_NAME + ".apk")

OVERRIDE = """
    /** 105% 界面大小分支：dp/sp 整体 ×1.05（幂等，构建脚本临时注入） */
    @Override
...
def patch_sources():
    ...
    s = s.replace('android:versionName="9.86 · 酷安@Eartecd"',
                  'android:versionName="' + VER_NAME + ' · 105%"')
    s = s.replace('android:label="简洁天气"', 'android:label="简洁天气 · 105%"')
...
try:
    ...
    sh([AAPT, "package", "-f", "-m", "-J", GEN, "-M", MANIFEST, "-S", RES,
        ...
        "--rename-manifest-package", "com.simpleweather.app.p105"])
